...
C = np.array(([7, 2])).reshape(-1,1) 

#Section
D = (A+B*k)/(k+1)
E=  (A+C*k)/(k+1)

ar1=0.5*(LA.norm(np.cross((A-E).T,(A-D).T)))
ar2=0.5*(LA.norm(np.cross((A-C).T,(A-B).T)))
print(ar1/ar2)
...
#if using termux
plt.savefig('chapters/10/7/4/6/figs/fig.pdf')
subprocess.run(shlex.split("termux-open chapters/10/7/4/6/figs/fig.pdf"))
#else
#plt.show()
