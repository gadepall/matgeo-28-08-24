...
ar1=0.5*(LA.norm(np.cross((A-B).T,(B-C).T)))
ar2=0.5*(LA.norm(np.cross((A-D).T,(D-C).T)))
print(ar1+ar2)
...
#if using termux
plt.savefig('chapters/10/7/3/4/figs/fig.pdf')
subprocess.run(shlex.split("termux-open chapters/10/7/3/4/figs/fig.pdf"))
#else
#plt.show()
